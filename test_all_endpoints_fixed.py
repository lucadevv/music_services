...
def execute_curl(method, endpoint, headers=None, data=None):
    ...
    try:
        ...
        # Parsear código de estado (última línea)
        status_code = stderr_output.split('\n')[-1] if stderr_output.split('\n') else "000"
        ...
        try:
            ...
            # Si falla, intentar extraer JSON de la respuesta cruda
            try:
                # Buscar patrón JSON {...}
                ...
                if json_match:
                    ...
                    # Si no hay JSON, crear objeto con output crudo
                    response = {"raw_output": output[:500]}
            except:
                # Si todo falla, usar output crudo
                response = {"raw_output": output[:500]}
...
if "status_code" in uploads_endpoint and uploads_endpoint["status_code"] != "501":
    inconsistency_data.append({
        "ID": "UPLOADS-1",
        "Endpoint": f"/uploads",
